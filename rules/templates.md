...
When converting to PDF (using the `/pdf` skill or pandoc), font handling is the most common failure point — especially for bilingual (Chinese + English) documents.
...
#### Additional PDF settings for mixed documents

```yaml
# Improve CJK line breaking and spacing
geometry: "margin=2.5cm"
linestretch: 1.4          # slightly more line height for CJK readability
fontsize: 11pt
```

#### LaTeX formulas
Use standard notation only — no custom packages. `amsmath` is available by default in pandoc's LaTeX template.

#### Tables
Max 5 columns; keep cell content narrow. CJK text in table cells is wider than Latin — reduce column count further if cells contain Chinese.

#### Code blocks
Add `--listings` flag to pandoc for better code block handling in PDF. CJK characters inside code blocks may break — if course code contains Chinese comments, test the PDF output early.
